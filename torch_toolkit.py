...
def add_dimension_glasso(var, dim=0):
    """Calculates L1 loss according to the gradient on the inputs
    """
    return var.pow(2).sum(dim=dim).add(1e-8).pow(1/2.).sum()

def deep_lasso_regularizer(loss, inputs):
    """Calculates deep lasso regularization loss
...
class NumericalTabSet(Dataset):
    
    def __init__(self, X, y, scaler=None):
        self.cont_mean_std = torch.tensor(X.agg(["mean", "std"]).T.values)
        self.conts = torch.tensor(X.values, dtype=torch.float32)
        self.unique_cats = (0)
        self.X = X
        self.y = y
        if scaler:
    ...
    def __getitem__(self, idx):
        X = self.conts[idx, :]
        y = torch.tensor(self.y, dtype=torch.float32)[idx]
        return X, y
    
def ft_transformer_training(
        loader : "DataLoader", 
        model : "Model", 
        loss : "Loss", 
        optimizer : "Optim", 
    ...
    model.train()
    t_loss = 0.
    for conts, y in loader:
        optimizer.zero_grad()
        res = model(torch.empty(conts.shape[0], 0).to(device), conts.to(device))
        res = res.to("cpu")
...
def ft_transformer_testing(
    ...
    for x, y in loader:
        x = x.to(device)
        null_tensor = torch.empty(x.shape[0], 0).to(device)
        out = model(null_tensor, x)
        out = out.to("cpu").flatten()
        r2_batch = r2_torch(target=y, preds=out.squeeze())
        mae_batch = mae_torch(target=y, preds=out.squeeze())
        r2_res.append(r2_batch.item())
        mae_res.append(mae_batch.item())
        x = x.to("cpu")
        del null_tensor
        torch.cuda.empty_cache()

    return np.mean(r2_res), np.mean(mae_res)
...
def deep_lasso_training(
    ...
    """
    Training function for a pyTorch regression model with deep lasso regularization.
    The corresponding loss value will be returned.
    ...
    for x, y in loader:
        ...
        reg = deep_lasso_regularizer(train_loss, x)
        train_loss = deep_lasso_weight * reg + (1 - deep_lasso_weight) * train_loss 
...
def create_kfold(
    ...
    """
    K-Fold Method to split an existing Dataframe in n splits based on the KFold implementation from sklearn.
    Returns a list containing n tuples of Train and Validation Dataframe
    Paramters:
        n_splits : Amount of splits
        shuffle : Shuffle index before splitting
    """
    kfold = KFold(n_splits=n_splits, shuffle=shuffle, random_state=42)
    train_val_dfs = []
    for train_index, test_index in kfold.split(X=df):
        train_df = df.iloc[train_index, :].reset_index(drop=True)
        val_df = df.iloc[test_index, :].reset_index(drop=True)     
        train_val_dfs.append((train_df, val_df))
    return train_val_dfs


# ---------------------------- Visualization ------------------------
# ToDo : Delete if not used  - If used an independent python will should be used as its not pytorch
    
def plot_confusion_matrix(cm, title, xticks=None, yticks=None, save_path=None, dpi=150, cmap="jet"):
    plt.figure(figsize=(8,6), dpi=dpi)
    if xticks and yticks:
        sns.heatmap(cm, annot=True, linewidths=.5, cmap=cmap, xticklabels=xticks, yticklabels=yticks)
    else:
        sns.heatmap(cm, annot=True, linewidths=.5, cmap=cmap)
...
def scatter_plot(pred, truth, title):
    tmp = pd.DataFrame({"Predicted" : pred, "Ground Truth" : truth})
    fig = px.scatter(
        tmp,
        x="Predicted",
        y="Ground Truth",
...
def train_val_test_scatters(train_pred, train_truth, val_pred, val_truth, test_pred, test_truth, title, height=800, width=800, save_path=None, show=True):
    # Organizing data in Dataframes for plotly
    preds = [train_pred, val_pred, test_pred]
    truths = [train_truth, val_truth, test_truth]
    labels = ["Train", "Val", "Test"]
    dfs = [pd.DataFrame({"Predicted" : pred, "Ground Truth" : truth}) for pred, truth in zip(preds, truths)]
    ...
    fig.for_each_annotation(lambda trace : trace.update(text=""))
    fig.for_each_yaxis(lambda trace : trace.update(title=""))
